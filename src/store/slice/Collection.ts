...
export const fetchAllMovies = createAsyncThunk('movies/getAll', async (obj:any)=>{
    ...
    const page = obj.page;
    const type = 'movie';
    const response = await MovieAPI.get(`?apikey=da6c85db&s=${searchString}&page=${page}&type=${type}`);
    return response.data;
})

export const fetchAllSeries = createAsyncThunk('series/getAll', async ()=>{
    const searchString = 'tamil';
    const page = 1;
    const type = 'series';
    const response = await MovieAPI.get(`?apikey=da6c85db&s=${searchString}&page=${page}&type=${type}`);
    return response.data;
...
// export const { movieList } = CollectionSlice.actions;
